...
function NavIndicator({activeIndex}:{activeIndex:number}){
  const x=useMotionValue(0);const w=useMotionValue(0);
  const springX=useSpring(x,{stiffness:300,damping:30});
  const springW=useSpring(w,{stiffness:300,damping:30});
  return <motion.div className="absolute bottom-0 h-[2px] bg-pink rounded-full" style={{x:springX,width:springW}}/>;
}
    ...
    >
      <div className={`mx-auto flex max-w-7xl items-center justify-between rounded-2xl border border-black/15 px-4 py-3 backdrop-blur-xl transition-all duration-500 ${scrolled?'bg-paper/95 shadow-lg shadow-black/5 border-black/10':'bg-paper/85'}`}>
        ...
        <nav className="hidden items-center gap-7 md:flex">
          {links.map(([href,key],idx)=>
            <Link key={href} href={href} onMouseEnter={()=>setActiveIdx(idx)}
              className="group relative text-sm font-semibold py-1">
              <Localized path={key}/>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={()=>setOpen(!open)} className="relative rounded-full border border-black/20 p-2 md:hidden transition-transform duration-200 active:scale-95">
        ...
        {links.map(([href,key],i)=>
          <motion.div key={href} initial={{opacity:0,x:-20,filter:'blur(4px)'}} animate={{opacity:1,x:0,filter:'blur(0px)'}} transition={{delay:i*.06,ease:[.2,.8,.2,1]}}>
            <Link onClick={()=>setOpen(false)} className="flex items-center justify-between border-b border-black/10 py-4 text-lg font-semibold hover:text-pink transition-colors" href={href}>
              <Localized path={key}/>
              <ArrowUpRight size={17} className="text-black/30 transition-transform group-hover:translate-x-0.5"/>
            </Link>
          </motion.div>
        )}
      </motion.div>}
    </AnimatePresence>
    <div className="fixed bottom-4 left-1/2 z-40 flex -translate-x-1/2 rounded-full border border-black bg-black px-2 py-2 text-white shadow-xl md:hidden glass-dark">
      {links.slice(0,4).map(([href,key])=>
        <Link key={href} href={href} className="px-4 py-2 text-[10px] font-bold transition-all duration-200 hover:text-pink active:scale-95">
          <Localized path={key}/>
        </Link>
      )}
    </div>
